...
public class Parser {
	...
	public static Lines getStationResults(Station station){
		...
		XMLParser parser = new XMLParser();
		String towards; // String för towards tagg?
		String lineNo;
		Calendar depTime;
		...
		if (xml!=null){
			...
			for (int i = 0; i < nl.getLength(); i++) { //Iterate all Line elements
				...
				towards = parser.getValue(e, "Towards"); //  XML towards, Översätter towards taggen, element Line "Towards"
				...
				l.setLine(lineNo);
				l.setTowards(towards); // Säger/ Sätter till att l.setTowards är towards från XML filen?
